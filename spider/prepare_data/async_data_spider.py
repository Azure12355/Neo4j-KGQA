...
class CrimeSpiderAsync:
    ...
    async def basicinfo_spider(self, url):
    ...
    async def treat_spider(self, url):
        html = self.get_html(url)
        selector = etree.HTML(html)
    ...
    async def drug_spider(self, url):
        html = self.get_html(url)
        selector = etree.HTML(html)
    ...
    async def food_spider(self, url):
        html = self.get_html(url)
        selector = etree.HTML(html)
    ...
    async def symptom_spider(self, url):
        html = self.get_html(url)
        selector = etree.HTML(html)
    ...
    async def inspect_spider(self, url):
        html = self.get_html(url)
        selector = etree.HTML(html)
    ...
    async def common_spider(self, url):
        html = self.get_html(url)
        selector = etree.HTML(html)
        ps = selector.xpath('//p')
        infobox = []
    ...
    async def spider_main(self):
        ...
        for page in range(1, 11000):
            ...
            tasks.append(self.process_page(page, basic_url, cause_url, prevent_url, symptom_url, inspect_url, treat_url,
                                           food_url, drug_url))

        results = await asyncio.gather(*tasks)
        # 使用异步插入到MongoDB
        await self.col.insert_many(results)

    async def process_page(self, page, basic_url, cause_url, prevent_url, symptom_url, inspect_url, treat_url, food_url,
                           drug_url):
        data = {'url': basic_url}
        data['basic_info'] = await self.basicinfo_spider(basic_url)
        data['cause_info'] = await self.common_spider(cause_url)
        data['prevent_info'] = await self.common_spider(prevent_url)
        data['symptom_info'] = await self.symptom_spider(symptom_url)
        data['inspect_info'] = await self.inspect_spider(inspect_url)
        data['treat_info'] = await self.treat_spider(treat_url)
        data['food_info'] = await self.food_spider(food_url)
        data['drug_info'] = await self.drug_spider(drug_url)
    ...
    def start(self):
        ...
        loop.run_until_complete(self.spider_main())


# 实例化并启动爬虫
if __name__ == '__main__':
    handler = CrimeSpiderAsync()
    handler.start()
