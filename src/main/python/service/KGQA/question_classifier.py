import ahocorasick
from src.main.NLPM.ner_model.predict_ner import BERT_CRF, PRETRAINED_MODEL
import os
import sys
...
class QuestionClassifier:
    def __init__(self):
        self.bert_crf = BERT_CRF(PRETRAINED_MODEL)  # BERT_CRF模型
        # self.sbert = SBert()   # SBERT模型
        resources_path = Utils.RESOURCES_PATH
        # 　特征词路径
        self.disease_path = os.path.join(resources_path, 'dict/disease.txt')
        self.department_path = os.path.join(resources_path, 'dict/department.txt')
        self.check_path = os.path.join(resources_path, 'dict/check.txt')
        self.drug_path = os.path.join(resources_path, 'dict/drug.txt')
        self.food_path = os.path.join(resources_path, 'dict/food.txt')
        self.producer_path = os.path.join(resources_path, 'dict/producer.txt')
        self.symptom_path = os.path.join(resources_path, 'dict/symptom.txt')
        self.deny_path = os.path.join(resources_path, 'dict/deny.txt')
        # 加载特征词
        ...
        print("KGQA_ChatBot->QuestionClassifier->初始化成功...")
    ...
    def classify(self, question):
        ...
        if not medical_dict:
            # 如果构造领域词典失效采用BERT模型
            medical_dict = self.bert_crf.formatting_output(
                self.bert_crf.predict_med_ner(question)
            )

        # # 如果BERT_CRF模型仍然没有预测出对应的实体信息,则使用SBERT进行智能语义识别匹配对应实体信息
        # if not medical_dict:
        #     entity = self.sbert.search(question)
        #     medical_dict = self.bert_crf.formatting_output(
        #         self.bert_crf.predict_med_ner(entity)
        #     )
